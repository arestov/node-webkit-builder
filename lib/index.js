...
var downloadPlatformFiles = function(base, options, platform, platform_name) {
    ...
    var indexFilePath = path.resolve(options.cacheDir, self._version.version,  'modes_index-' + platform_name + '.json');

    return Downloader.checkCache(platform.cache, platform.cv_files, indexFilePath)
        .then(function(ok) {
            if (ok) {
                self.emit('log', 'Using cache for: ' + platform_name);
                platform.modes_index = ok;
                return true;
            } else {
                self.emit('log', 'Downloading: ' + platform.url);
                return Downloader.downloadAndUnpack(platform.cache, platform.url, indexFilePath)
                    .then(function(info) {
                        platform.modes_index = ok;
...
var getReleaseFolder = function(reqb, base, options) {
    ...
    return reqb('app_name').then(function() {
        ...
        base.temp_folder = path.join(base.rootReleasePath, 'temp');

        if (!fs.existsSync(base.temp_folder)) {
            fs.mkdirpSync(base.temp_folder);
        }
...
var addMode= function(obj, file, platform) {
    var modes_index = platform.modes_index;
    var key = path.relative(platform.cache, file);
    if (modes_index.hasOwnProperty(key)) {
        obj.mode = modes_index[ key ];
    }
    return obj;
};
var addWKFile = function(files_map, index, platform, file, key, obj) {
    var item = addMode(obj, file, platform);
    index[key] = item;
    files_map.push(item);
};
var nwkit_zipstruc = {
        ...
        osx: function(reqp, reqb, platform, platform_name, base, options) {
            ...
            platform.files_list.forEach(function(file) {
                if (file.indexOf(deep_root) === 0) {
                    ...
                    addWKFile(files_map, index, platform, file, relpath, {
                        src: file,
    ...
    default: function(reqp, reqb, platform, platform_name, base, options) {
        ...
        platform.app_filename = app_filename;

        platform.files_list.forEach(function(file) {
           
            if (!renamed && file === oldpath) {
                renamed = true;
                addWKFile(files_map, index, platform, file, app_filename, {
                    src: file,
...
var getMapItem = function(platform, key) {
    var item = platform.zipstruc.index[key];
    if (!item) {
        console.log('there is no map_item for: ' + key);
    }
    return item;
};
var platforms_modifications = {
    index: {
        osx: function(reqp, reqb, platform, platform_name, base, options) {
            var array = [
                [path.join('Contents', 'Resources', 'nw.icns'), options.macIcns],
                [path.join('Contents', 'Resources', 'Credits.html'), options.macCredits],
                [path.join('Contents', 'Info.plist'), options.macPlist, function(map_item) {
                    return Promise.all([reqb('user_files'), reqb('app_name'), reqb('release_folder')]).then(function() {
                        var defaultPlist = {
                            appName: base.appName,
                            appVersion: base.appVersion,
                            copyright: base._appPkg.copyright || false
                        };

                        var plistOptions = (options.macPlist ? _.defaults(options.macPlist, defaultPlist) : defaultPlist);
                        var src_path = map_item.src;
                 
                        var output_path = path.join(base.temp_folder, 'Info.plist');
                        

                        return Utils.editPlist(src_path, output_path, plistOptions).then(function() {
                            map_item.src = output_path;
                        });
                    });
            ...
            var handler = function(key, new_dest, func) {
                if (typeof new_dest === 'string') {
                    var map_item = getMapItem(platform, key);
                    map_item.src = new_dest;
                } else if (typeof func === 'function') {
                    return func(getMapItem(platform, key));
                }
            };
            ...
            array.forEach(function(item) {
                var promise = handler.apply(null, item);
        ...
        win: function(reqp, reqb, platform, platform_name, base, options) {
            if (!options.winIco) {return Promise.resolve();}

            // Set icon
            base.emit('log', 'Update executable icon');
            return reqb('release_folder').then(function() {

                var output_temp_path = path.join(platform.temp_folder, 'iconed_app.exe');

                var map_item = platform.zipstruc.files_list[0];


                return Utils.copyFile(map_item.src, output_temp_path).then(function() {
                    return new Promise(function(resolve, reject) {
                        winresourcer({
                            operation: "Update",
                            exeFile: output_temp_path,
                            resourceType: "Icongroup",
                            resourceName: "IDR_MAINFRAME",
                            lang: 1033, // Required, except when updating or deleting 
                            resourceFile: path.resolve(options.winIco)
                        }, function(err) {
                            if(err) {
...
function(reqp, reqb, platform, platform_name, base, options) {}
    */
    ...
    zip_nwfiles_struc: function(reqp, reqb, platform, platform_name, base, options) {
        var args = arguments;
        return Promise.all([reqp('nodewebkit_files_list'), reqp('app_path_shift'), reqb('app_name')])
            .then(function() {
                var handler = selectHandler(platform_name, nwkit_zipstruc);
                platform.zipstruc = handler.apply(null, args);
                console.log(platform.zipstruc.list[0]);
                console.log(platform.zipstruc.list[2]);
            });
    },
    platform_modifications: function(reqp, reqb, platform, platform_name, base, options) {
        var args = arguments;
        return reqp('zip_nwfiles_struc').then(function() {
            return selectHandler(platform_name, platforms_modifications).apply(null, args);
        });
    },
    ...
    zip_app: function(reqp, reqb, platform, platform_name, base, options) {
        return Promise.all([reqp('zip_nwfiles_struc'), reqp('zip_userfiles_sctruc'), reqp('platform_modifications')]).then(function() {
            var all_files = platform.zipstruc.list.concat(platform.zip_userfiles_map);
            return new Promise(function(resolve, reject) {
...
NwBuilder.prototype.downloadNodeWebkit = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        if(!Downloader.checkCacheSync(platform.cache, platform.files)) {
            downloads.push(
