...
var getQueue = function(limit) {
    var copyFileQueue = [];
    var queueCounter = 0;
    var checkQueue = function(){
        if (queueCounter < limit && copyFileQueue.length) {
            var cur = copyFileQueue.shift();
            queueCounter++;
            var promise = cur.request();
            promise.finally(function(){
                --queueCounter;
                checkQueue();
            });
            promise.then(function() {
                cur.defer.resolve.apply(cur.defer, arguments);
            }, function(err) {
                cur.defer.reject.apply(cur.defer, arguments);
            });
        }
    };
...
module.exports = {
    ...
    getFileList: function(fileglob) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            srcFiles.forEach(function(internalFileName) {
    ...
    generateZipFile: function (files, _event) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            archive.on('error', function(e) {
                console.log('NO ZIPPPP' + e)
            });

            // Add the files
            files.forEach(function (file) {
                var data = { name: file.dest };
                if (typeof file.mode !== 'undefined') {
                    data.mode = parseInt(file.mode, 8);
                }
                archive.file(file.src, data);
            });
